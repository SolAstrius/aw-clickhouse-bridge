...
pub type QueryFn =
    fn(args: Vec<DataType>, env: &VarEnv, ds: &dyn crate::QuerySource) -> Result<DataType, QueryError>;
...
mod qfunctions {
    ...
    pub fn query_bucket(
        ...
        let events = match ds.get_events(
            ...
            Ok(events) => events,
            Err(crate::QuerySourceError::NoSuchBucket(_)) => {
